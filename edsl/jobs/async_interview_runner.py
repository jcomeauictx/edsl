...
class AsyncInterviewRunner:
    ...
    async def _run_single_interview(
        ...
        try:
            # Debug: Interview starting
            start_time = time.time()
            print(
                f"DEBUG: Interview {idx} STARTING with model {interview.model.model} at {start_time:.3f}"
            )

            await interview.async_conduct_interview(self.run_config)

            # Debug: Interview completed
            end_time = time.time()
            duration = end_time - start_time
            print(
                f"DEBUG: Interview {idx} COMPLETED with model {interview.model.model} in {duration:.3f}s"
            )
            ...
            print(
                f"DEBUG: Interview {idx} FAILED with model {interview.model.model} after {duration:.3f}s - Error: {str(e)}"
            )
